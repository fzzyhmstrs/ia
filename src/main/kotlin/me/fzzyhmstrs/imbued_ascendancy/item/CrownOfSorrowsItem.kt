...
class CrownOfSorrowsItem(settings: Settings): AbstractAugmentJewelryItem(settings), DamageTracking,KillTracking, AugmentTracking {
    ...
    override fun onWearerDamaged(
        ...
        if (rnd < getSorrowChance(stack,IaConfig.items.crownOfSorrows.defense50Percent.get())){
            val nbt2 = stack.orCreateNbt
            nbt2.putBoolean("active", true)
            nbt2.putInt("active_timer",IaConfig.items.crownOfSorrows.activeDuration.get())
            if (rnd < getSorrowChance(stack,IaConfig.items.crownOfSorrows.regret50Percent.get())){
