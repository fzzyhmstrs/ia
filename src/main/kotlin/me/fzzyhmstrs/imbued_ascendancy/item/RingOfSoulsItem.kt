...
class RingOfSoulsItem(settings: Settings): AbstractAugmentJewelryItem(settings), KillTracking, Modifiable {
    ...
    private val tierLeveler = TierLeveler(IaConfig.items.ringOfSouls.killTierMultiplier.get(),IaConfig.items.ringOfSouls.baseKillsPerTier.get())
    ...
    override fun appendTooltip(stack: ItemStack, world: World?, tooltip: MutableList<Text>, context: TooltipContext) {
        ...
        if (nbt.contains("tier") && nbt.contains("amount")){
            ...
            if (tier == IaConfig.items.ringOfSouls.maxTier.get()){
                tooltip.add(AcText.translatable("item.viscerae.ring_of_souls.tier_max",IaConfig.items.ringOfSouls.maxTier.get()).formatted(Formatting.GOLD))
            } else {
            ...
            tooltip.add(AcText.translatable("item.viscerae.ring_of_souls.tier","1","0",IaConfig.items.ringOfSouls.baseKillsPerTier.get()).formatted(Formatting.GOLD))
        }
    }
    ...
    override fun onWearerKilledOther(stack: ItemStack, wearer: LivingEntity, victim: LivingEntity, world: ServerWorld) {
        ...
        nbt.putInt("tier", min(tier,IaConfig.items.ringOfSouls.maxTier.get()))
    ...
    companion object{
        private val SOUL_MODIFIER_SCEPTER = AugmentModifier(Identifier(IA.MOD_ID,"souls_modifier_scepter_tier_1")).withDamage(0.5f).also { ModifierRegistry.register(it) }
        private val SOULS_MODIFIER = EquipmentModifier(Identifier(IA.MOD_ID,"souls_modifier_tier_1"), persistent = true, randomSelectable = false).withModifiers(
            SOUL_MODIFIER_SCEPTER.modifierId).also { ModifierRegistry.register(it) }
        init{
            if (IaConfig.items.ringOfSouls.maxTier.get() > 1){
                for (i in 2..IaConfig.items.ringOfSouls.maxTier.get()){
                    val soulModifierScepterTierX = AugmentModifier(Identifier(IA.MOD_ID,"souls_modifier_scepter_tier_$i")).withDamage(0.5f*i)
                    ModifierRegistry.register(soulModifierScepterTierX)
                    val soulModifierTierX = EquipmentModifier(Identifier(IA.MOD_ID,"souls_modifier_tier_$i"), persistent = true, randomSelectable = false).withModifiers(
